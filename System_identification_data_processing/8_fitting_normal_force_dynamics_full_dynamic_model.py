from functions_for_data_processing import get_data, plot_raw_data, process_raw_vicon_data,plot_vicon_data\
    ,model_parameters,full_model_with_pitch_and_roll_dynamics,throttle_dynamics,steering_dynamics,dyn_model_culomb_tires,generate_tensor_past_actions,full_dynamic_model
...
train_x = torch.cat((train_x_model_inputs, train_x_past_acc_x,train_x_past_acc_y), dim=1)

# -- Y lables --
# measured accelerations in the body frame
train_y = torch.unsqueeze(torch.tensor(np.concatenate((df['ax body'].to_numpy(),df['ay body'].to_numpy(),df['aw_abs_filtered_more'].to_numpy()))),1).cuda()


#instantiate the model
dt_int_steering =  np.diff(df['vicon time'].to_numpy()).mean() / refinement_factor
full_model_with_pitch_and_roll_dynamics_obj = full_model_with_pitch_and_roll_dynamics(initial_guess,dt_int_steering,n_past_actions*refinement_factor,lr,lf)

#define loss and optimizer objects
loss_fn = torch.nn.MSELoss() 
optimizer_object = torch.optim.Adam(full_model_with_pitch_and_roll_dynamics_obj.parameters(), lr=learning_rate)
...
for i in tqdm_obj:
    ...
    acc_x,acc_y,acc_w, pitch_unscaled, roll_unscaled = full_model_with_pitch_and_roll_dynamics_obj(train_x)
    # stack vertically
...
[w_natural_Hz_pitch,k_f_pitch,k_r_pitch,w_natural_Hz_roll,k_f_roll,k_r_roll]= full_model_with_pitch_and_roll_dynamics_obj.transform_parameters_norm_2_real()
