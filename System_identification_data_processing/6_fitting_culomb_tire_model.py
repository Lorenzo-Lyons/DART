...
long_term_predictions = False # decide if you want to produce long term predictions or not
...
delay_vicon_to_robot = 0 #0.1 #0.05 #[s]


l = 0.175
# recording while driving around the track
...
# recording 4 september 2024 0.6 need to go up
theta_correction = +0.5/180*np.pi 
lr = 0.135 #0.1175 #0.5*l    # reference point location taken by the vicon system
COM_positon = 0.0925 #measuring from the rear wheel
l_COM = lr - COM_positon #distance of the reference point from the centre of mass)
# NOTE that we don't really know the exact location of the COM, but if we assume that the wheel should have the same curve,
# we can tweek this parameter until the two curves are the same (assuming no other mistakes in the front steer angle ecc)
...
elif folder_path == 'System_identification_data_processing/Data/8_circles_rubbery_floor_1_file':
    pass
...
m = 1.67
Jz_0 = 0.006513 # uniform rectangle of shape 0.18 x 0.12

# correcting the moment of inertia for the extra distance of the reference point from the COM
Jz = Jz_0 + m*l_COM**2
...
# Front wheel
scatter_front = ax10.scatter(V_y_f_wheel, Fy_f_wheel_vec, color = "#72a0c1", label='front wheel data',alpha=0.5,s=3)
# Rear wheel
scatter_rear = ax10.scatter(V_y_r_wheel, Fy_r_wheel_vec,  color = "#cc92c2", label='rear wheel data',alpha=0.5,s=3)
...
# --------------- fitting tire model--------------- 
print('')
print('Fitting linear culomb friction tire model ')
...
train_x = torch.unsqueeze(torch.tensor(np.concatenate((V_y_f_wheel,V_y_r_wheel))),1).cuda()
train_y = torch.unsqueeze(torch.tensor(np.concatenate((Fy_f_wheel_vec,Fy_r_wheel_vec))),1).cuda() 
...
# #instantiate the model
# pacejka_culomb_tire_model_obj_f = culomb_pacejka_tire_model(initial_guess)

# #define loss and optimizer objects
# loss_fn = torch.nn.MSELoss() # reduction = 'mean'
# optimizer_object = torch.optim.Adam(pacejka_culomb_tire_model_obj_f.parameters(), lr=learning_rate)
...
#     # compute fitting outcome with current model parameters
#     output = pacejka_culomb_tire_model_obj_f(train_x)
...
# # --- print out parameters ---
# [d_f,c_f,b_f,e_f] = pacejka_culomb_tire_model_obj_f.transform_parameters_norm_2_real()
# d_f, c_f,b_f,e_f= d_f.item(), c_f.item(), b_f.item(), e_f.item()
# print('Front Wheel parameters:')
# print('d_f = ', d_f)
# print('c_f = ', c_f)
# print('b_f = ', b_f)
...
# #instantiate the model
# pacejka_culomb_tire_model_obj_r = culomb_pacejka_tire_model(initial_guess)

# #define loss and optimizer objects
# loss_fn = torch.nn.MSELoss() # reduction = 'mean'
# optimizer_object = torch.optim.Adam(pacejka_culomb_tire_model_obj_r.parameters(), lr=learning_rate)
...
#     # compute fitting outcome with current model parameters
#     output = pacejka_culomb_tire_model_obj_r(train_x)
...
# # --- print out parameters ---
# [d_r,c_r,b_r,e_r] = pacejka_culomb_tire_model_obj_r.transform_parameters_norm_2_real()
# d_r, c_r,b_r,e_r= d_r.item(), c_r.item(), b_r.item(), e_r.item()
...
plt.plot(df['vicon time'].to_numpy(),Fy_r_wheel_vec,label='Fy_r')
plt.plot(df['vicon time'].to_numpy(),Fy_r_wheel_vec + Fy_f_vec,label='Fy_sum')
# net moment
plt.plot(df['vicon time'].to_numpy(),-(Fy_r_wheel_vec)*lr + Fy_f_vec*lf,label='Angular Moment')
plt.legend()
...
# lateral_force_vec_f = pacejka_culomb_tire_model_obj_f(v_y_f_wheel_plotting).detach().cpu().numpy()

# # evaluate model on plotting interval
# v_y_r_wheel_plotting = torch.unsqueeze(torch.linspace(np.min(V_y_r_wheel),np.max(V_y_r_wheel),100),1).cuda()
# lateral_force_vec_r = pacejka_culomb_tire_model_obj_r(v_y_r_wheel_plotting).detach().cpu().numpy()
...
# # Rear wheel
# scatter_rear = ax6.scatter(V_y_r_wheel, Fy_r_wheel_vec, c=df['steering angle time delayed'].to_numpy(), cmap='viridis', label='data')
